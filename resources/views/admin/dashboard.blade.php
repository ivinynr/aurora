<x-layout.admin titulo="Dashboard">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
        ...
        <x-ui.stat-card
            ...
            valor="{{ $resumo['total_instituicoes'] }}"
                    ...
                    @forelse($doacoesRecentes as $doacao)
                        <div class="px-6 py-4 flex items-center gap-4 hover:bg-cream-100 transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-bark-400">{{ $doacao->instituicao->nome }} &bull; {{ $doacao->created_at->diffForHumans() }}</p>
                            </div>
                            <span class="text-sm font-bold text-sage-400">R$ {{ number_format($doacao->valor, 2, ',', '.') }}</span>
                        </div>
                    @empty
        ...
        <div>
            <x-ui.card :hover="false" padding="none">
                <div class="p-6 border-b border-cream-200">
                    <h2 class="font-serif text-lg font-bold text-bark-800">Top Instituições</h2>
                </div>

                <div class="p-4 space-y-3">
                    @forelse($instituicoesMaisArrecadadas as $inst)
                        <div class="p-3 rounded-xl bg-cream-100">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-semibold text-bark-700 truncate">{{ $inst->nome }}</span>
                                <span class="text-xs font-bold text-terra-500 shrink-0">
                                    R$ {{ number_format($inst->valor_arrecadado, 2, ',', '.') }}
                                </span>
                            </div>
                            @if($inst->meta > 0)
                                <x-ui.barra-progresso :percentual="$inst->percentualArrecadado()" tamanho="sm" />
                            @endif
                        </div>
                    @empty
                        <p class="text-sm text-bark-400 text-center py-4">Nenhuma instituição cadastrada.</p>
                    @endforelse
                </div>
            </x-ui.card>
        </div>
